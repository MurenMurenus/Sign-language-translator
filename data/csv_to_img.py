...
for i in range(len(image_names)):
    ...
    image_name_iter = str(int(image_names[i]))+str(iter) + '.png'
    if not os.path.exists(f'sign_language_dataset/mnist/{image_name}'):
        os.makedirs(f'sign_language_dataset/mnist/{image_name}')
    image.save(f'sign_language_dataset/mnist/{image_name}/{image_name_iter}_mnist')
    iter += 1
